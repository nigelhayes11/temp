...
class MonoHybridScraper:
    ...
    def fetch_assets(self):
        """Aktif Referer domainini ve stream sunucusunu bulur."""
        active_referer = None

        print("🌐 Aktif monotv domaini taranıyor...")
        for i in range(530, 580):
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    print(f"✅ Aktif Referer: {active_referer}")
                    break
            except:
                continue
    ...
    def run(self):
        ...
        if not stream:
            print("❌ Stream sunucusu alınamadı.")
            return
        ...
        for cid, name in self.kanallar.items():
            m3u.append(
                f'#EXTINF:-1 group-title="İNATSPOR TV",{name}'
            )
            m3u.append(
        ...
        print(f"🏁 Tamamlandı: {len(self.kanallar)} kanal yazıldı.")
